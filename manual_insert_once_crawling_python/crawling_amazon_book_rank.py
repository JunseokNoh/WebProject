...
class AmazonBookCrawling(crawling.Crawling, ABC):
    ...
    def connect_db(self, i, book_title, book_info_url, book_author, image_url, tmp6, tmp7, tmp8):
        ...
        if int(rank_number) == 1:
            sql = """delete from amazon_book_rank"""
            curs.execute(sql)

        sql = """insert into amazon_book_rank (rank, title, url, author, image_url) values (%s, %s, %s, %s, %s)"""
        curs.execute(sql, (rank_number, book_title, book_info_url, book_author, image_url))
