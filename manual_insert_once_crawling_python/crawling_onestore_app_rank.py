...
class OnestoreAppCrawling(crawling.Crawling, ABC):
    ...
    def crawler(self):
        try:
            ...
            f.write("table : onestore_app_rank UPDATED" + "\n")
            print("table : onestore_app_rank UPDATED")
            f.close()
    ...
    def connect_db(self, i, name, info_url, image_url, publisher, rank_type, tmp7, tmp8):
        ...
        if count == 1:
            sql = """delete from onestore_app_rank"""
            curs.execute(sql)
            count += 1

        sql = """insert into onestore_app_rank (rank, name, rank_type, url, publisher, image_url) values (%s, %s, %s, %s, %s, %s)"""
        curs.execute(sql, (rank_number, name, rank_type, info_url, publisher, image_url))

        # sql = """select name from onestore_app_rank where rank = %s and rank_type = %s"""
        # curs.execute(sql, rank_number, rank_type)
        # row = curs.fetchone()
        # if row[0] == name:
        #     # print("same onestore app")
        #     pass
        # else:
        #     # print("change value " + str(rank_number) + " : " + title)
        #     sql = """update onestore_app_rank set name=%s, url=%s, image_url=%s, publisher=%s where rank=%s and rank_type = %s"""
