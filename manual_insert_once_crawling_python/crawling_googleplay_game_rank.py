...
class GoogleplayGameCrawling(crawling.Crawling, ABC):
    ...
    def crawler(self):
        try:
            ...
            f.write("table : googleplay_game_rank UPDATED" + "\n")
            print("table : googleplay_game_rank UPDATED")
            f.close()
    ...
    def get_url(self, URL):
        ...
        if soup[0].get_text() == "Google Play":
            return soup[4].find("a")["href"]
        elif len(soup) > 5 and soup[5].get_text() == "Google Play":
            return soup[9].find("a")["href"]
        elif len(soup) > 10 and soup[10].get_text() == "Google Play":
    ...
    def connect_db(self, i, name, info_url, image_url, publisher, rank_type, tmp7, tmp8):
        ...
        if count == 1:
            sql = """delete from googleplay_game_rank"""
            curs.execute(sql)
            count += 1

        sql = """insert into googleplay_game_rank (rank, name, rank_type, url, publisher, image_url) values (%s, %s, %s, %s, %s, %s)"""
        curs.execute(sql, (rank_number, name, rank_type, info_url, publisher, image_url))

        # sql = """select name from googleplay_game_rank where rank = %s and rank_type = %s"""
        # curs.execute(sql, rank_number, rank_type)
        # row = curs.fetchone()
        # if row[0] == name:
        #     # print("same googleplay game")
        #     pass
        # else:
        #     # print("change value " + str(rank_number) + " : " + title)
        #     sql = """update googleplay_game_rank set name=%s, url=%s, image_url=%s, publisher=%s where rank=%s and rank_type = %s"""
