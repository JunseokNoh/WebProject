...
class NaverMovieRatingCrawling(crawling.Crawling, ABC):
    ...
    def connect_db(self, i, movie_title, movie_rating, movie_info_url, image_url, director_name, actor_names, tmp8):
        ...
        if rank_number == 1:
            sql = """delete from naver_movie_rating_rank"""
            curs.execute(sql)

        sql = """insert into naver_movie_rating_rank (rank, title, rating, url, image_url, director_name, actor_names) values (%s, %s, %s, %s, %s, %s, %s)"""
        curs.execute(sql, (rank_number, movie_title, movie_rating, movie_info_url, image_url, director_name, actor_names))
